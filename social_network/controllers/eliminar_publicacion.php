...
if ($row['usuario_id'] !== $user_id) {
    echo json_encode(['success' => false, 'error' => 'No tienes permisos para eliminar esta publicación']);
    exit;
}
...
while ($archivo = $resultado_archivos->fetch_assoc()) {
    $archivo_path = "../uploads/" . $archivo['archivo_url']; // Ruta del archivo
...
$tablas_relacionadas = ['archivos_proyectos', 'imagenes_proyectos', 'valoraciones', 'comentarios', 'retweets', 'proyectos_categorias'];
