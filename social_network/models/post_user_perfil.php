...
<?php
    ...
    // Consulta para obtener los proyectos del usuario al que se accede
    $query = "SELECT p.id, p.titulo, p.descripcion, p.fecha_publicacion, u.nombre, u.apellido, u.foto_perfil, 
                        ...
                        (SELECT COUNT(*) FROM retweets r WHERE r.proyecto_id = p.id AND r.usuario_id = {$_SESSION['id']}) AS user_retweeted,
    ...
    $resultado = $conn->query($query);
     ...
     if ($resultado->num_rows > 0) {
        while ($row = $resultado->fetch_assoc()) {
            ...
            echo "<div style='max-width: 600px; margin: 0 auto;' class='post card mb-3'>";
